...
function counter() {
	var startNum = prompt("Введите начальное значение счетчика");
	var step = prompt("Введите шаг отсчета");
	var countLoop = prompt("Введите количество итераций");
	
	if (startNum === null || step === null || countLoop === null)  return alert("Вы вышли из prompt");

	if (Number.isNaN(+startNum) || Number.isNaN(+step) || Number.isNaN(+countLoop)) return alert("Вы ввели не число");
	...
	return function (){	
		...
		for (var i = startNum, j = 0; j < countLoop; j++, i += step) {
			result = i;
		}
		return result;
	}
...
alert("Итоговое значение счетчика" + ": " + resCounter());
